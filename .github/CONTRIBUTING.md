...
- **A layout engine**, for anything that touches pagination, the table of
  ...
  - **LibreOffice** on any platform, *plus the UNO bridge*. On Debian and
    Ubuntu two pieces ship separately: `sudo apt install python3-uno
    libreoffice-math`. Without the Math component every formula converts to
    blank space in the PDF, silently.
...
`libreoffice-writer`, `libreoffice-math`, `python3-uno`, `fonts-liberation` and
`qpdf` installed, so the checks that skip on a bare machine do run there. It then lints and builds the bundled
